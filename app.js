...
(function () {
  ...
  const navLinks = document.querySelectorAll(
    ".header-links a, .header-actions-desktop a, .dropdown-item"
  );
  ...
  if (mobileToggle && nav) {
    ...
    // Mobile panel ke nav links — panel band karo click pe
    // Dropdown triggers ko ignore karo
    const mobileNavLinks = document.querySelectorAll(
      ".mobile-nav-panel .header-links a, .mobile-nav-panel .dropdown-item"
    );

    mobileNavLinks.forEach((link) => {
      link.addEventListener("click", () => {
        nav.classList.remove("nav-open");
        // Sab dropdowns bhi band karo
...
(function () {
  ...
  document.querySelectorAll(".nav-dropdown").forEach(function (dropdown) {
    var trigger = dropdown.querySelector(".btn-nav-dropdown");
    if (!trigger) return;
  ...
  // Panel ke bahar tap kare toh sab band
  ...
  // Desktop pe hover se dropdown — kuch change nahi
})();